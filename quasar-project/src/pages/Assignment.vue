    ...
    <!--<q-dialog v-model="demo" persistent>
      <q-card style="min-width: 350px">
        ...
        <q-card-actions align="right" class="text-primary">
          ...
          <q-btn @click="deleteStud(row)" flat label="Yes" v-close-popup  />
        </q-card-actions>
      </q-card>
    </q-dialog>-->
...
<script>
import { roundWbSunny } from '@quasar/extras/material-icons-round';
import { ref } from 'vue'
export default {
  setup () {
 ...
 Editstud:ref({ 
    ...
    methods:{
        ...
        EditItem(){
          ...
          .then(function(response){
            if(response.data.ok){
              self.$q.notify({'message':'student Updated'})
              self.get_stud();
              self.prompt=false;
            }
        ...
        deleteStud(row){
          ...
          .then(function(response) {
          if (response.data.ok) {
          self.$q.notify({ message: 'Student Deleted', color: 'positive' });
          self.get_stud();
      }
    })
